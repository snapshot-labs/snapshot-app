...
function FollowersScreen({ route }: FollowingScreenProps) {
  ...
  useEffect(() => {
    getWalletFollowers(address, setWalletFollowers);
  }, []);
  ...
  return (
    ...
    >
      ...
      >
        <BackButton
          title={i18n.t("addressFollowers", { address: shorten(address) })}
        />
      </View>
      <ScrollView
        style={[common.screen, { backgroundColor: colors.bgDefault }]}
      >
        {walletFollowers.map((follow, i) => {
          return (
            <TouchableOpacity
              onPress={() => {
                navigation.push(USER_PROFILE, {
                  address: follow.follower,
                });
              }}
            >
              <View
              ...
              >
                <UserAvatar address={follow.follower} size={24} />
                <Text
                  style={{
